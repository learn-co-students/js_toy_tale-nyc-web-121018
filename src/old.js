...
toyContainer.addEventListener('click', (event) => {
  if (event.target.dataset.action === 'edit') {
    ...
    let likeNum = event.target.previousElementSibling
     ...
     likeToy(event.target.dataset.id, parseInt(likeNum.innerText)).then(console.log(" likes"))
...
function likeToy(toyId, toyData) {
  ...
  return fetch("http://localhost:3025/toys/" + toyId, {
    method: 'PATCH',
    headers: {
      "Content-Type": "application/json"
    },
...
const editToy = document.querySelector('#edit-toy-form')
// add event listener when user hits EDIT TOY button
editToy.addEventListener("submit", (e) => {
    const toyNameInput = document.querySelector("#edit-name").value
    const toyImageInput = document.querySelector("#edit-image").value
    const editedToy = document.querySelector(`#toy-${toyData.id}`)
